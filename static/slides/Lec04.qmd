...
:::
::: {.column width='50%'}
On the right we have defined an S3 method called `report`, it is designed to return a message about the type/mode/class of an object passed to it.

* Try running the `report` function with different input types, what happens? 

* Now comment out the code for `report.integer` and try rerunning the code, what has changed? 

* What does this tell us about S3, types, modes, and classes?

* What if we also comment out `report.double`?
:::
::::
